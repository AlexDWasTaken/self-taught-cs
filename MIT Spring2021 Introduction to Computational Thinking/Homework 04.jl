...
student = (name = "AlexD", kerberos_id = "I don't know")
...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 2:m
		l = max(seam[i-1]-1, 1)
		r = min(seam[i-1]+1, n)
		value, index = findmin(energies[i, l:r])
		index = seam[i-1] + (-1, 0, 1)[index + (seam[i-1] == 1)]
		seam[i] = index
	end
...
function least_energy(energies, i, j)
	...
	l = max(j-1, 1)
	r = min(j+1, n)
	value, index = findmin([least_energy(energies, i+1, newj)[1] for newj in l:r])
	energy = value + energies[i, j]
	index = j + (-1, 0, 1)[index + (j==1)]
...
# ╔═╡ 6fb25a78-de6a-453f-89ec-8bfabb8bd978
least_energy(grant_example, 3, 2)

# ╔═╡ 9c14057c-ddf4-41f3-8763-b38f69e7cbf3
function least_energy_backwards(energies, i, j)
	m, n = size(energies)
	if i == 1
		return (energies[i, j], j)
	end
	l = max(j-1, 1)
	r = min(j+1, n)
	_, direction = findmin(energies[i-1, l:r])
	index = j + (-1, 0, 1)[direction + (j==1)]
	energy = least_energy_backwards(energies, i-1, index)[1] + energies[i, j]
...
function recursive_seam(energies, starting_pixel)
	...
	steps = zeros(Int, m)
	steps[1] = starting_pixel
	for i in 2:m
		steps[i] = least_energy(energies, i, steps[i-1])[2]
	end
...
# ╔═╡ dda5595f-de3f-4f26-bb74-480d8daba4a1
md"""
⬇️My answer is actually correct. There's actually more than one optimal seams.
"""
...
function memoized_least_energy(energies, i, j, memory::Dict)
	...
	if haskey(memory, (i, j))
		...
		value, index = 
			findmin([
				memoized_least_energy(energies, i+1, newj, memory)[1] 
				for newj in l:r
			])
		energy = value + energies[i, j]
		index = j + (-1, 0, 1)[index + (j==1)]
		memory[(i, j)] = energy, index
		return energy, index
	end
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	steps[1] = starting_pixel
	for i in 2:m
		steps[i] = memoized_least_energy(energies, i, steps[i-1], memory)[2]
	end
...
# ╔═╡ ee37ceb3-f49a-452c-b6fe-0664943bd903
md"""
⬇️My answer is actually correct. There's more than one optimal seam.
"""
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix{Union{Nothing,Tuple{Float64,Int}}})
	...
	if memory[i, j] != nothing
		...
		value, index = 
			findmin([
				matrix_memoized_least_energy(energies, i+1, newj, memory)[1] 
				for newj in l:r
			])
		energy = value + energies[i, j]
		index = j + (-1, 0, 1)[index + (j==1)]
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	steps[1] = starting_pixel
	for i in 2:m
		steps[i] = matrix_memoized_least_energy(energies, i, steps[i-1], memory)[2]
	end
...
# ╔═╡ bd027874-37a1-4261-9144-d024397a5565
md"""
# I have done these in my draft.jl.
"""
