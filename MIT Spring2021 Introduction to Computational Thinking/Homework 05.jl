...
ten_twelve = FirstRankOneMatrix(collect(1:10), collect(1:12)) # Your code here
...
# methods(size) #finished, but this cell is too long.

# ╔═╡ fada4734-8505-11eb-3f2b-d1f1ef391ba4
function Base.size(M::FirstRankOneMatrix)
	result = (size(M.v)[1], size(M.w)[1])
	return result 
end
...
begin
	a = RankOneMatrix([1, 2, 3], [4, 5, 6])
	a.v
end

# ╔═╡ d3e7c8d1-4b4a-47b6-9c96-150333078f42
begin
	b = RankOneMatrix([1, 2, 3])
	b.v
end
...
function Base.getindex(M::LowRankMatrix, i, j)
	...
	return sum([getindex(subM, i, j) for subM in M.Ms]) # Your code here
end
...
function matvec(M::LowRankMatrix, x)
	...
	return sum([matvec(subM, x) for subM in M.Ms]) # Your code here
end
...
function numerical_rank(A::AbstractMatrix; tol=1e-5)
	Sₐ = svd(A).S
	return count(x -> !isless(x, tol), Sₐ)
end
...
function k_rank_ones(k, m, n)
	...
	generated = nothing
	while generated_rank != k
		generated = sum([RankOneMatrix(rand(m), rand(n)) for i in 1:k])
		generated_rank = numerical_rank(generated)
	end
	return generated
end
