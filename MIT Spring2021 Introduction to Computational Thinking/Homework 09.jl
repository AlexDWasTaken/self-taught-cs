...
function trajectory(w::Coordinate, n::Int)
	...
	positions = accumulate(+, steps, init = w)
...
function trajectory(c::Coordinate, n::Int, L::Number)
	steps = rand(possible_moves, n)
	positions = accumulate( (x, y) -> collide_boundary(x+y, L) , steps, init = c)
...
let
	origin = Coordinate(0, 0)
	trajectories = [trajectory(origin, 1000) for _ in 1:10]
	p = plot(ratio=1)

	for route in trajectories
		plot_trajectory!(p, route)
	end
...
function initialize(N::Number, L::Number)
	...
	for i in 1:N
		x = rand(-L:L)
		y = rand(-L:L)
		while used[x+L+1, y+L+1]
			x = rand(-L:L)
			y = rand(-L:L)
		end
		used[x+L+1, y+L+1] = true
		new_agent = Agent(Coordinate(x, y), S)
		push!(agents, new_agent)
	end
	selected = rand(1:N)
		
	agents[selected] = Agent(
			agents[selected].position,
			I
		)
...
# ╔═╡ 86cc85ac-e4cf-48ba-a8f0-309702dda5a0
md"""
⬇️I'm sure I'm correct.
"""
