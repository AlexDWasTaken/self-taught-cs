...
function counts2(data::Vector)
	counted = counts(data)
	ks, vs = collect(keys(counted)), collect(values(counted))
	perm = sortperm(ks)
...
# ╔═╡ eecf9558-f274-45b5-a7de-be60227dadb8
f(x) = return

# ╔═╡ 6127a6fa-51b8-4eea-88dc-26ad492100b9
typeof(f(1))
...
function geometric(p::Real)
	if p <= 0 || p > 1 throw(ArgumentError("p cannot generate a geometric random variable")) end
	time_taken = 1
	while(!bernoulli(p))
		time_taken += 1
	end
...
The patient recovers instantly.
"""
...
let
	xs, ps = probability_distribution(large_experiment)
	logged_ps = -log.(ℯ, ps)
	bar(xs, logged_ps, alpha=0.5, leg=false)	
end
...
# ╔═╡ 28083c48-8e72-4041-b0c9-ff540f0c1dcc
md"""
Here is the case where p = $(p), N = $(N)
"""

# ╔═╡ 48751015-c374-4a77-8a00-bca81bbc8305
my_experiment = experiment(p, N)

# ╔═╡ 562202be-5eac-46a4-9542-e6593bc39ff9
let
	xs, ps = probability_distribution(my_experiment)
	bar(xs, ps, alpha=0.5, leg=false)	
end
...
τ(p) = let
	data = experiment(p, 10000)
	recovery = sum(data) / 10000
	return recovery
end
...
let
	p = collect(0.02:0.02:1)
	mean = τ.(p)
	bar(p, mean, alpha=0.5)
	title!("mean versus p")
end
...
let
	p = collect(0.02:0.02:1)
	mean = τ.(p)
	logged_mean = log.(ℯ, mean)
	bar(p, logged_mean, alpha=0.5)
	title!("logₑmean versus p")
end
...
let
	p = collect(0.02:0.02:1)
	mean = τ.(p)
	logged_logged_mean = log.(ℯ, log.(ℯ, mean))
	bar(p, logged_logged_mean, alpha=0.5)
	title!("logₑlogₑmean versus p")
end
...
```math
\sum_{k=1}^{\infty} P_k = \lim _ {n \to \infty} p \cdot 
\frac{{1-p}^n}{1 - (1-p)} = 1
...
let
	xs, ps = probability_distribution(my_experiment)
	bar(xs, ps, alpha=0.5, leg=true, fillcolor = :red)
	bar!(Ps, alpha = 0.5, fillcolor = :blue)
end
...
let
	xs, ps = probability_distribution(my_experiment)
	errors = Ps .- ps[1:length(Ps)]
	bar(errors, alpha = 0.5, fillcolor = :blue)
end
...
function cumulative_sum(xs::Vector)
	...
	for i in 2:length(xs)
		@inbounds result[i] = xs[i] + result[i-1]
	end
	return result
end
