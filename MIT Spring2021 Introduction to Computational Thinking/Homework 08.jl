...
# ╔═╡ ca811cd9-bdfb-4b3a-93ff-3e5d9cf78cd6
md"""
# Some figures are broken here. I don't know how to fix them.
"""
...
student = (name = "AlexD", kerberos_id = "IDontHaveOne")
...
function tangent_line(f, a, h)
	res = x ->
		(x - a) * finite_difference_slope(f, a, h) + f(a)
	return res
end
...
function euler_integrate(fprime::Function, fa::Number, 
	...
	for i in 2:length(T)
		@inbounds append!(
			result, euler_integrate_step(fprime, result[i-1], T[i], h)
		)
	end
...
function euler_SIR(β::Number, γ::Number, sir_0::Vector, T::AbstractRange)
	...
	for i in 2:num_steps
		push!(
			result, euler_SIR_step(β, γ, result[i-1], h)
		)
	end
...
# ╔═╡ 5979116e-32e3-457d-97e8-6a59115994e2
@bind γ Slider(0:0.001:0.3,show_value = true, default = 0.15)

# ╔═╡ c8ea057f-037f-4fbb-b19a-87cca69b618c
@bind β Slider(0:0.025:0.5,show_value = true, default = 0.3)
...
When the final answer stopped moving.
"""
...
function gradient_descent_2d(f, x0, y0; η=0.01)
	...
	x_previous, y_previous = x0, y0
	x, y = gradient_descent_2d_step(f, x0, y0; η = η)

	while abs(x_previous - x) + abs(y_previous - y) > η * 1e-3
		x_previous, y_previous = x, y
		x, y = gradient_descent_2d_step(f, x_previous, y_previous; η = η)
	end
...
# ╔═╡ 0332a3f5-23bd-4e75-8d57-86921a9e9c29
...
function loss_dice(μ, σ)
	steps = length(dice_y)
	loss = 0
	@simd for i in 1:steps
		@inbounds loss += (gauss(dice_x[i], μ, σ) - dice_y[i]) ^ 2
	end
	return loss
end
...
found_μ, found_σ = let
	μ, σ = gradient_descent_2d(loss_dice, guess_μ, guess_σ)	
	μ, σ
end
...
function loss_sir(β, γ)
	loss = 0
	predict = euler_SIR(β, γ, [0.99, 0.01, 0.00], 1:1000)
	S = map(x -> x[1], predict)
	I = map(x -> x[2], predict)
	R = map(x -> x[3], predict)
	SS = map(x -> x[1], spatial_results)
	II = map(x -> x[2], spatial_results)
	RR = map(x -> x[3], spatial_results)
	loss += sum(
		(SS .- S).^2
	)
	loss += sum(
