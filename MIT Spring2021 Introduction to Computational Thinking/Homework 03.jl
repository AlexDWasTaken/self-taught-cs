...
student = (name = "AlexD", kerberos_id = "Idon'thaveanid")
...
The Continental Divide of the Americas is located in the Rocky Mountains and designates the line at which waters flow either to the Atlantic or Pacific Oceans. Triple Divide Peak (2,440 m or 8,020 ft) in Glacier National Park is so named because water falling on the mountain reaches not only the Atlantic and Pacific but Hudson Bay as well. Farther north in Alberta, the Athabasca and other rivers feed the basin of the Mackenzie River, which has its outlet on the Beaufort Sea of the Arctic Ocean.
...
function clean(text)
	text = filter(isinalphabet,lowercase(unaccent(text)))
...
unused_letters = [] # replace with your answer
...
th_frequency = sample_freq_matrix[findfirst(isequal('t'), alphabet), 					findfirst(isequal('h'), alphabet)]
...
# ╔═╡ 0dd1a09d-3b31-4f92-abe6-f109cac59941
most_likely_to_follow = t ->
alphabet[
	findfirst(
		isequal(
			maximum(sample_freq_matrix[findfirst(isequal(t), alphabet), : ]) 
		),
		sample_freq_matrix[findfirst(isequal(t), alphabet), : ]
	)
]

# ╔═╡ 58b664ea-6408-43a8-9a57-9a0f48c375ea
sample_freq_matrix[findfirst(isequal('w'), alphabet), : ]
...
# ╔═╡ b91f75af-59f2-40ae-9da7-39939c09be18
most_likely_to_precede = t ->
alphabet[
	findfirst(
		isequal(
			maximum(sample_freq_matrix[:, findfirst(isequal(t), alphabet)]) 
...
most_likely_to_precede_w = most_likely_to_precede('w') # replace with your answer
...
row_sums = [sum(sample_freq_matrix[i, :]) for i in 1:size(sample_freq_matrix)[2]]

# ╔═╡ 4a0314a6-7dc0-4ee9-842b-3f9bd4d61fb1
col_sums = [sum(sample_freq_matrix[:, i]) for i in 1:size(sample_freq_matrix)[1]]
...
function matrix_distance(A, B)
	C = abs.(A .- B)
	res = sum(C)
	return res # do something with A .- B
end
